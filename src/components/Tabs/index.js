...
class Tabs extends React.PureComponent {
  constructor(props) {
    super(props);
    let value = Array.isArray(props.value) ? props.value : props.value ? [props.value] : [];
  ...
  handleClick = (item) => (e) => {
    if (!this.DetermineDisabled(item)) {
      const {multi, showAll, items, userCtrlKey} = this.props;
      let checkedValue = this.state.value;
      if ((userCtrlKey && multi && e.ctrlKey) || (multi && !userCtrlKey)) {
        // 可多选
        ...
        if (item.key === 'ALL') {
          ...
          if (checkedValue.indexOf(item.key) > -1) {
            ...
            checkedValue = Array.isArray(checkedValue) ? checkedValue : [checkedValue];
          } else {
      ...
      this.setState({value: [...checkedValue]}, () => {
        ...
        typeof onChange === 'function' && onChange(multi ? checkedValue : checkedValue[0]);
      });
    }
  };
  ...
  renderTabItem = (item) => {
    ...
    isDisabled = disabledTip[item.key] ? isDisabled : `${isDisabled ? 'ss-tab-item-disabled-pure' : ''}`;
    return (
      <label
        key={item.key}
        className={`${isDisabled} ${isChecked} ${isDisabled && disabledTip[item.key] ? 'tooltip' : ''}`}
        style={{width: itemWidth ? itemWidth : ''}}
        data-msg={isDisabled && disabledTip[item.key] ? disabledTip[item.key] : null}
  ...
  render() {
    ...
    if (showAll) {
      options = [{key: 'ALL', value: showAllText ? showAllText : '全部'}, ...items];
    }

    return <div className={`ss-tab ss-tab-${size ? size : 'normal'}`}>{options.map(this.renderTabItem)}</div>;
  }
}
